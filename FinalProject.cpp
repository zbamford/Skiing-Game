...
double viewAzimuth = 0.05;	// Angle of view up/down (in radians)
...
const double Zmin = -30.0, Zmax = 10.0;   // Control z dimensions of the viewable scene
...
double ZextraDistance = -15.0;              // Extra distance we have moved to/from the scene
...
void myRenderScene() {
    ...
    std::vector<std::pair<float, float>> locs = RenderScene(xPos, zPos);
    ...
    for (int i = 0; i < locs.size(); i++) {
        loc = locs[i];
        cout << "loc:(" << xPos << ", " << zPos << ")" << endl;
        cout << "vec:(" << (xPos - loc.first) << ", " << (-zPos - loc.second) << ")" << endl;
        dist = sqrt((xPos - loc.first) * (xPos - loc.first) + (-zPos - loc.second) * (-zPos - loc.second));
...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Final Project", NULL, NULL);
	if (window == NULL) {
    ...
    printf("Press or hold A to move left and D to move right.");
